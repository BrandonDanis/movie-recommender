...
module.exports = {
                ...
                module.exports.setFavouriteGenres(username, favGenres, function (result) {
                    if (result['status'] == 200) {
                        ...
                        module.exports.postRatings(username, movieIDs, massRatings, function (result) {
							if (result['status'] == 200) {
								module.exports.completeSetup(username, function (result) {
									callback(result);
								});
							} else {
								callback(result);
	...
	completeSetup: function (username, callback) {
		db.update('users').set({'status': 'complete'}).where({'username': username}).returning('*').row(function (err, row) {
			if (!err) {
				if (row != null) {
					callback({status:200});
				} else {
					callback({status:404, error: 'Failed to finish'});
				}
			} else {
				console.error(err);
				callback({status:500, error:'Database error'});
