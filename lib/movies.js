	...
	findSpecificMovie: {
		...
		byId: function(movieId,callback) {
            ...
            db.select('m.id', 'm.overview', 'm.release_date', 'm.poster', 'm.title', 'g.name AS genre',
                ...
                .on('mc.cast_id', 'c.id').where({'m.id': movieId}).rows(function (err, rows) {
                if (!err) {
                    if (rows[0] != null) {
                        movie['id'] = rows[0]['id'];
            ...
            /*db.raw('SELECT id, overview, release_date, poster, title FROM movies WHERE movies.id = $1', [movieId]).rows(function (err, rows) {
                if (!err) {
                    if (rows[0] != null) {
                        movie = rows[0];
                        db.select('name').from('genres').join('movies_genres').on('genres.id', 'movies_genres.genre_id').join('movies').on('movies_genres.movie_id', 'movies.id').where({'movies.id': movieId}).rows(function (err, rows) {
                            if (!err) {
                                if (rows[0] != null) {
                                    movie['genres'] = rows;
                                    db.select('name', 'imageurl').from('directors').join('movies_directors').on('directors.id', 'movies_directors.director_id').join('movies').on('movies_directors.movie_id', 'movies.id').where({'movies.id': movieId}).row(function (err, row) {
                                        if (!err) {
                                            if (row != null) {
                                                movie['director'] = row;
                                                db.select('name', 'imageurl').from('casts').join('movies_casts').on('casts.id', 'movies_casts.cast_id').join('movies').on('movies_casts.movie_id', 'movies.id').where({'movies.id':movieId}).rows(function (err, rows) {
                                                    if (!err) {
                                                ...
                                                callback({status:404, error:'No directors for movie found'})
                                            }
                                        } else {
                                            callback({status:500, error:'Database error (directors)'})
                                        }
                                    });
                                } else {
                                    callback({status: 404, error: 'No genres for movie found'});
                                }
                            } else {
                                callback({status: 500, error: 'Database error (genres)'})
                            }
                        });
					}else{
						callback({status: 404, error: 'No movie found'});
					...
					console.log(err);
					callback({status: 500, error: 'Database error (movies)'});
				}
			});*/
		},
