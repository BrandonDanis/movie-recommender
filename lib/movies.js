...
module.exports = {
    ...
    moviesByPopularity: function (callback) {
        db.raw('SELECT m.*, AVG(r.rating) AS average FROM movies m JOIN ratings r ON r.movie_id = m.id GROUP BY m.id ' +
            ...
            .rows(function (err, rows) {
                if (!err) {
                    ...
                    db.raw('SELECT m.*, AVG(r.rating) AS average FROM movies m LEFT JOIN ratings r ON r.movie_id = m.id ' +
                        'GROUP BY m.id ORDER BY average DESC;').rows(function (err, rows) {
                        if (!err) {
                            if (rows[0] != null) {
                                movies = movies.concat(rows);
                            }
	...
	genres: {
		getAll: function(callback) {
			db.raw('SELECT * FROM genres ORDER BY id').rows(function(err,rows){
				if(!err){
					if(rows[0] != null) {
		...
		getMoviesByGenreName: function(genreName, callback) {
			db.raw('SELECT movies.id,movies.overview,movies.release_date,movies.runtime,movies.poster,movies.title,genres.name AS genre_name FROM movies,genres,movies_genres WHERE genres.name = $1 AND movies.id = movies_genres.movie_id AND genres.id = movies_genres.genre_id',[genreName]).rows(function(err,rows){
				if(!err){
					if(rows[0] != null) {
						callback({status: 200, movies: rows});
					}else{
		...
		getMoviesByGenreId: function(genreId, callback) {
			db.raw('SELECT movies.id,movies.overview,movies.release_date,movies.runtime,movies.poster,movies.title,genres.name AS genre_name FROM movies,genres,movies_genres WHERE genres.id = $1 AND movies.id = movies_genres.movie_id AND genres.id = movies_genres.genre_id',[genreId]).rows(function(err,rows){
				if(!err){
					if(rows[0] != null) {
						callback({status: 200, movies: rows});
					}else{
			...
			module.exports.genres.getAll(function(genres) {
				if(genres['status'] == 200){
					...
					for(var i=0;i<genresArray.length;i++){
						...
						db.raw('SELECT m.id, m.poster, g.name FROM movies m,movies_genres mg,genres g WHERE mg.movie_id = m.id AND mg.genre_id = g.id AND g.name = $1 LIMIT 1',[genresArray[i]['name']]).rows(function(err,rows){
							if(!err){
    ...
    getPopularMultiGenre: {
		...
		getInitialList: function (genres, limit, callback) {
					...
					if (rows[0] != null) {
						module.exports.getRatings(rows, function (result) {
							if (result['status'] == 200 && rows.length != 10) {
								module.exports.getPopularMultiGenre.getMoreMovies(query, rows, genres, limit,
									function (result) {
										callback(result);
									});
							} else {
						...
						module.exports.getPopularMultiGenre.getMoreMovies(query, [], genres, limit, function (result) {
							callback(result);
						});
					}
				} else {
		...
		getMoreMovies: function (query, movies, genres, maximum, callback) {
			var limit = maximum - movies.length;
			console.log('getMoreMovies: ' + limit);
			db.raw(query.replace(' AND r.movie_id = m.id', '').replace(', ratings r', '').replace('LIMIT 10', 'LIMIT '
				+ limit)).rows(function (err, rows) {
                if (!err) {
                    if (rows[0] != null) {
                        if (movies.length != 0)
		...
		afterSetup: function (username, limit, callback) {
			db.select('genre_id').from('favourite_genres').where({username: username}).rows(function (err, rows) {
				if (!err) {
					if (rows[0] != null) {
						var genres = [];
						for (var i = 0; i < rows.length; i++) {
							genres = genres.concat(rows[i]['genre_id']);
						}
						module.exports.getPopularMultiGenre.getInitialList(genres, limit, function (result) {
							callback(result);
						});
					} else {
						callback({status:404, error:'User is not setup properly'});
					}
				} else {
					callback({status:500, error:'Database error'});
				}
