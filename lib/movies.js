...
module.exports = {
    ...
    getPopularMultiGenre: function (genres, callback) {
        var genreString = 'g.name = \'' + genres[0] + "\'";
        for (var i = 1; i < genres.length; i++) {
            genreString += " OR g.name = \'" + genres[i] + "\'";
        }

        db.raw('SELECT DISTINCT(m.id) FROM movies m,movies_genres mg,genres g, ratings r WHERE mg.movie_id = m.id AND mg.genre_id = g.id AND r.movie_id = m.id AND (' + genreString + ') LIMIT 10;').rows(function (err, rows) {
            if (!err) {
                if (rows[0] != null) {
                    var movieString = 'movie_id = ' + rows[0]['id'];
                    var json = [{id: rows[0]['id']}];
                    for (var i =1; i < rows.length; i++) {
                        movieString += " OR movie_id = " + rows[i]['id'];
                        json.push({id: rows[i]['id']});
                    }
