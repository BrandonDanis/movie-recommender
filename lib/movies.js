...
module.exports = {
	...
	getAllMovies: function(callback){
		db.raw('SELECT movies.id,movies.overview,movies.release_date,movies.runtime,movies.poster,movies.rating,movies.title,genres.name AS genre_name FROM movies,genres,movies_genres WHERE movies.id = movies_genres.movie_id AND genres.id = movies_genres.genre_id').rows(function(err,rows){
			if(!err){
				if(rows[0] != null) {
	...
	findSpecificMovie: {
		...
		allInfoById: function(movieId, callback) { //used for /movie page
			db.raw('SELECT * FROM movies,genres,movies_genres,directors,movies_directors,casts,movies_casts WHERE movies_genres.movie_id = movies.id AND movies_genres.genre_id = genres.id AND movies_directors.movie_id = movies.id AND movies_directors.director_id = directors.id AND movies_casts.movie_id = movies.id AND movies_casts.cast_id = casts.id AND movies.id = $1',[movieId]).rows(function(err,rows) {
				if(!err){
					if(rows[0] != null) {
						callback({status: 200, movie: rows});
					}else{
						callback({status: 404, error: 'No movie found with that movie id'});
					}
				}else{
					console.log(err);
					callback({status: 500, error: 'Database error'});
		...
		allInfoByTitle: function(movieTitle, callback) { //used for /movie page
